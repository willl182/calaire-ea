...
Para una medición de ozono de \(x=100\ \text{nmol/mol}\), calcular los coeficientes de sensibilidad y las contribuciones estándar debidas a temperatura y presión. Interpretar sus signos y comparar ambas contribuciones después de expresarlas en la unidad del mensurando. Además, completar especificación del mensurando, aclarar representatividad de T/P y ubicar una fuente no explícita en ecuación básica.
...
### 2.7 Especificación del mensurando y fronteras

Una respuesta completa puede ser:

> Fracción molar de O₃ en aire seco dentro de celda fotométrica, atribuida al volumen óptico efectivo, expresada en nmol mol⁻¹, obtenida mediante modelo Beer–Lambert con T y P medidas y promediada durante intervalo definido por procedimiento; línea y filtro aguas arriba quedan excluidos del mensurando fotométrico, mientras acondicionamiento interno pertinente queda incluido. Representatividad espacial y temporal de aire ambiente queda excluida.

Otra frontera puede ser válida si se declara y se modela de forma coherente. «Concentración de ozono» sin matriz, punto, unidad, procedimiento, promedio y alcance es insuficiente.

T y P deben representar condiciones reales del gas dentro de región pertinente de celda. Lectura de sensor solo sirve si ubicación, calibración y representatividad están demostradas. Si solo se conoce indicación del sensor, diferencia sensor–gas debe registrarse como vacío o componente.

Ejemplos de fuente no explícita:

- pérdida residual en línea como factor `f_linea` nominalmente cercano a uno;
- humedad residual como factor `f_h`;
- corrección de cero como término aditivo `b_0`.

No deben añadirse si quedan fuera del mensurando o ya están cubiertas por otra evaluación.

## 3. Resultado final

> **Resultado:** \(c_T=+0.335\ \text{nmol mol}^{-1}\text{K}^{-1}\), \(c_P=-0.987\ \text{nmol mol}^{-1}\text{kPa}^{-1}\), \(u_T(x)=0.0503\ \text{nmol/mol}\) y \(u_P(x)=0.0494\ \text{nmol/mol}\). Temperatura produce aumento y presión produce disminución del resultado calculado. Las contribuciones estándar son casi iguales; diferencia de aproximadamente \(0.0010\ \text{nmol/mol}\) no es relevante. Especificación y fronteras deben indicar celda, matriz, unidad, T/P, promedio e inclusión o exclusión de línea, filtro, acondicionamiento y representatividad.
...
- Completa mensurando y fronteras, explica qué representan T/P y ubica fuente no explícita sin duplicarla.
