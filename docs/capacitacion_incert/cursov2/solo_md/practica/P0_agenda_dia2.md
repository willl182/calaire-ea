# P0 — Agenda de la práctica de laboratorio complementaria

Jornada práctica que complementa el curso teórico (M1–M8, ver `../README.md` y `../diseno_curso_v2.md`). Ejecuta en instrumentos reales los protocolos que el curso teórico solo describe documentalmente. Referencias completas de cada experimento están en los archivos `E0x_*.md` de esta carpeta. **Duración real de P1: 7.5–9 h con dos subequipos en paralelo (O₃ y NOx); P2 añade 6.4–6.7 h adicionales** — ver el detalle de cómputo en las secciones P1 y P2 abajo (hallazgo A1 de la revisión de diseño).

## Prerrequisito logístico crítico: E08 se inicia ≥7 días antes

**E08 — Deriva de cero y span 24 h/7 d** es pasivo y de largo aliento: debe iniciarse **al menos 7 días antes** de la práctica de laboratorio complementaria, con verificaciones en `t=0`, `24 h` y `7 d` (idealmente diarias). Si no se inicia con antelación, el equipo pierde la evidencia de deriva que alimenta E15 y debe sustituirla por el límite documental del fabricante, declarándolo como limitación.
...
- Encender analizadores, calibrador y patrón la **noche anterior** a la práctica de laboratorio (o mínimo 2 h antes si no es posible dejarlos encendidos), para no consumir tiempo de jornada en calentamiento.
...
| 6 | **E15** — Presupuesto híbrido del equipo propio | M7 | 90 min | Entregable evaluado de la práctica de laboratorio (independiente del de M7); se ejecuta en conjunto con el subequipo NOx al converger las vías |
...
| Orden | Experimento | Módulo asociado | Duración activa | Nota |
|---:|---|---|---|---|
| 1 | **E04** — GPT y eficiencia del convertidor (sesión 1 de 2) | M6 | 100 min | Duración experimental: 2 días; ver nota abajo |
...
**E04 tiene una duración experimental de 2 días** (ver protocolo E04); esta duración pertenece al experimento, no a la estructura del curso: la sesión 1 se ejecuta dentro de P1; la sesión 2 (orden descendente, ~100 min) se agenda como continuación, idealmente el día siguiente a la práctica de laboratorio complementaria, o se sustituye por una nota de limitación en E15 si el curso no dispone de esa segunda sesión.
...
## Entregable mínimo por equipo al cierre de la práctica de laboratorio
...
## Referencias cruzadas con el curso teórico
...
- M7 → **E15**, presupuesto del equipo propio, construido con evidencia de ambas vías (O₃ y NOx). Es el entregable evaluado **de esta jornada**, separado del entregable de M7 (caso KRISS/BIPM), que se evalúa en el curso teórico y no se reemplaza.
