...
Construya el presupuesto Tipo B del analizador 49i para una indicación de 120 nmol/mol, usando numéricamente 120 ppb, escala completa de 200 ppb, coeficientes de sensibilidad unitarios e independencia entre componentes. Calcule el total, ordene las contribuciones, audite las filas marcadas como **SUPUESTO**, construya diagrama causal, clasifique cobertura/calidad de evidencia, proponga reemplazo de tres filas, describa presupuesto híbrido y evalúe cualitativamente dependencia con nivel. Realice comparación con APOA-370 sin formar total para este último.
...
| 2 | Certificado de calibración | \(U=1.8\) ppb, \(k=2\) | incertidumbre expandida, divisor \(k=2\) | 0.9000 | 0.8100 | **26.4** |
...
### 6. Diagrama causal, cobertura y plan de reemplazo

Diagrama mínimo defendible:

```text
Mensurando: fracción molar de O3 en aire seco a la salida del sistema de acondicionamiento y dentro de la celda del analizador, expresada en nmol/mol, promediada durante 60 s y atribuida mediante calibración vigente; línea externa y representatividad espacial/temporal quedan excluidas
├─ Respuesta instrumental: ruido, resolución, linealidad
├─ Estabilidad: deriva de cero, deriva de span
├─ Condiciones: presión
└─ Referencia/calibración: certificado
```

Cada fila debe enlazarse a una rama; ramas sin evidencia se marcan como vacío. Clasificación de calidad:

| Fila | Calidad actual | Cobertura principal | Reemplazo recomendado |
|---|---|---|---|
| Ruido RMS | Verificada para 60 s | Dispersión a promedio declarado | Precisión intermedia de QC bajo promedio real |
| Deriva de cero | Verificada para 24 h | Cambio aditivo corto | QC de ceros entre días/ciclos |
| Deriva de span | Supuesto/extrapolada | No demostrada a 24 h | Historia de verificaciones de span |
| Linealidad | Verificada como límite FS | Falta de ajuste limitada documentalmente | Verificación multipunto y residual |
| Presión | Faltante | No demostrada | Calibración sensor + diferencia sensor–celda |
| Resolución | Faltante | Incremento digital supuesto | Documento/ensayo y revisión contra repetibilidad |
| Certificado | Faltante | Valor de referencia supuesto | Certificado real y declaración de cobertura |

Plan válido selecciona tres reemplazos y explica qué filas se retiran para evitar doble conteo. Presupuesto híbrido futuro puede escribirse conceptualmente como

\[
u_c^2=u_{PI}^2+u_{patron}^2+u_{\text{externas no cubiertas}}^2,
\]

sin sumar nuevamente ruido, deriva, operador o ambiente si ya están incluidos en \(u_{PI}\).

### 7. Dependencia con nivel
...
respuesta cualitativa esperada:

- 10 nmol mol⁻¹: término constante \(u_0\) probablemente domina;
- 120 nmol mol⁻¹: ambos pueden ser relevantes, según valores;
- 180 nmol mol⁻¹: término proporcional \(u_r c\) gana peso y puede dominar.

Sin valores de \(u_0\) y \(u_r\), no se afirma cruce numérico. Cerca de cero no se usa incertidumbre relativa como resumen único y se revisan límites físicos.

### 8. Comparación prudente con el APOA-370
...
- entrega diagrama causal, cobertura/calidad de evidencia, reemplazo de tres filas, estructura híbrida y dominancia cualitativa a 10, 120 y 180 nmol mol⁻¹.
