...
3. ajustar, **con ceros y niveles**, una regresión `lectura_uut = intercepto + pendiente × X` por ciclo; la regresión global se conserva solo como diagnóstico opcional del instructor;
...
6. completar matriz de cobertura para residuos, variación entre ciclos, certificado y ceros pre/post.
...
### Paso 4. Diagnóstico opcional del instructor: regresión global

Como diagnóstico complementario no exigido en el entregable, pueden reunirse las 24 observaciones, sin sustituir las regresiones por ciclo:
...
### Paso 7. Matriz de cobertura

| Dato | Fuente que cuantifica | Qué cubre | Qué no cubre |
|---|---|---|---|
| Residuos dentro de ciclo | Regresión OLS por ciclo | Dispersión y falta de ajuste local bajo puntos/condiciones ensayados | Incertidumbre del patrón, estabilidad de meses, montaje futuro |
| Variación entre ciclos | Tres ciclos independientes | Estabilidad corta durante verificación | Deriva histórica entre meses, otros operadores/equipos |
| Certificado | Calibración superior | Valor corregido de referencia y \(U(X)\) declarada | Pérdidas o montaje local salvo declaración explícita |
| Ceros pre/post | Verificación actual | Cambio aditivo corto y estabilidad de cero dentro de ciclo | Pendiente, interferencias, deriva histórica completa |

Matriz no autoriza sumar cuatro cifras como independientes. Residual, repetibilidad y precisión intermedia pueden solaparse. Incertidumbre del patrón no queda incluida automáticamente en OLS.

### Paso 8. Conformidad razonada
...
```r
archivo <- "datasets/dataset_verificacion_multipunto.csv"
...
- **Completa:** corrige con \(X=(I_{ref}+0.4)/1.003\), conserva `lectura_ref`, calcula diferencias en las 24 filas, usa ceros y niveles en las regresiones por ciclo, informa residuos, estabilidad, falta de ajuste alto, completa matriz de cobertura y concluye conforme con comprobaciones previas al ajuste. Ajuste global es opcional.
- **Parcial:** llega a la decisión correcta, pero omite alguna operación, los ceros, la deriva, matriz de cobertura o análisis de residuos.
...
13. Usar límites de conformidad como incertidumbre estándar.
14. Sumar residual, repetibilidad y precisión intermedia sin revisar solapamiento.
15. Suponer que incertidumbre del patrón está incluida automáticamente en regresión OLS.
16. Extrapolar función fuera del intervalo calibrado.
