...
r <- -1
delta <- 0.5 * 10^r
h <- 50000L             # trials por bloque
min_bloques <- 4L       # mínimo para estimar dispersión entre bloques
max_bloques <- 40L      # límite de seguridad: máximo 2 000 000 trials
p_cobertura <- 0.95
...
cat("------------------------------------------------------------\n")
cat(sprintf("h = %d trials/bloque; delta = %.3f nmol/mol\n", h, delta))
