...
- **JCGM 101:2008** es Suplemento 1: propagación de distribuciones mediante método de Monte Carlo (MCM) para modelos con una magnitud de salida.
- **JCGM 102:2011** es Suplemento 2: extensión a cualquier número de magnitudes de salida, incluidas matrices de covarianza y regiones de cobertura multivariantes.
...
Como ejemplo bajo supuestos explícitos, fotometría UV basada en Beer–Lambert puede escribirse
...
donde \(u(x_i,x_j)\) es covarianza. Con coeficiente de correlación \(r_{ij}\):
...
Puede usarse cualquier generador de números pseudoaleatorios validado, con periodo, calidad y reproducibilidad adecuados. Deben registrarse software, versión, algoritmo relevante y, cuando reproducibilidad lo requiera, semilla. La calidad del modelo y de las PDFs suele importar más que la elección entre generadores modernos validados.
...
1. Elegir probabilidad de cobertura \(p\) y tolerancia numérica \(\delta\).
...
2. Formar intervalo GUF \([y_{GUF}-U_p,\ y_{GUF}+U_p]\), o aproximación correspondiente.
...
## 7.2 Forma de expresar resultado
